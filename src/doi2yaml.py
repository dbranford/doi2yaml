...
class ResultParser:
    def parse_to_yaml(self, result: Result) -> str:
        doi = result.doi
        yaml_args = []
        yaml_args.append(result.to_yaml_key("title", ["title", 0]))
        yaml_args.append(result.to_yaml_key("journal", ["container-title", 0]))
        yaml_args.append(
            result.to_yaml_key("authors", "author", processor=parse_authors)
        )

        yaml_args.append(
            result.to_yaml_key(
                "date", ["published", "date-parts", 0], processor=format_date
            )
        )
        yaml_args.append(result.to_yaml_key("volume", ["volume"]))
        yaml_args.append(
            result.to_yaml_key(
                "page", ["article-number"], fallback={"key_yaml": "page"}
            )
        )
